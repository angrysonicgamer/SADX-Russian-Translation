...
void ev004B_t_eggmantakesbirdie_tw(int state)
{
	...
	case 1:
		...
		EV_CameraAng(1, 0, 0xC00, 0x8100, 0x100);
		...
		EV_Wait(60);
		EV_CameraAng(1, 0, 0xF600, 0xC00, 0xFD00);
		...
		EV_MoveRotation(amy, 985, 0x3DFD, 0);
		...
		EV_CameraAng(1, 0, 0xF900, 0xC800, 0);
		...
		EV_Wait(1);
		EV_CameraAng(1, 0, 59293, 12359, -0x100);
		EV_CameraPos(1, 0, 36.25f, 1557.0f, 3247.6499f);
		EV_CameraAng(1, 58, 2717, 12359, 0xFE00);
		...
		EventSe_Stop(0);
		EV_MsgClose();
		EV_MoveRotation(sonic, 0, 0x2E00, 0);
		EV_LookFree(sonic);
		...
		stopObjectAll();
		EV_Wait(1);
		EV_SetAng(KOTORI, 0xA00, 0xBF00, 0);
		EV_ClrAction(KOTORI);
